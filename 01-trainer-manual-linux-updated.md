...
Trainer emphasis: this distinction matters later when diagnosing services. A shell-launched background process belongs to that interactive session; a systemd-managed service is supervised independently by PID 1.
...
A useful teaching connection is:
...
For beginners, stress the distinction between:
- **loop** = repetition;
- **conditional** = decision;
- **function** = reusable unit of logic.

The backup exercise deliberately combines all three, which is why it is a useful teaching example rather than just a scripting syntax exercise.
...
- **`ss`** shows sockets and listening ports. Teach trainees to ask: “What is listening, on which address/port, and which process owns it?” This is much more actionable than simply seeing that a network problem exists.
...
When a trainee says “the application isn't reachable,” avoid jumping straight to firewall commands. Walk down the stack:
...
For a failed service, teach the sequence conceptually as:
...
The important trainer concept is that **WSL2 is both a Linux environment and a Windows-integrated environment**. Day 1 establishes the environment; Day 4 proves that the pieces still work together before Vertica is introduced.
...
## 10. Trainer guidance: how the four Linux days fit together

The four days form a progression rather than four unrelated command lists:

**Day 1 — Filesystem and permissions**  
Trainees learn where things live, how paths work, how files/directories are manipulated, and how access is controlled.

**Day 2 — Multi-user Linux and running software**  
They move from static files to users, groups, processes, jobs and system services.

**Day 3 — Automation and software lifecycle**  
They turn individual commands into scripts, then learn how software is installed and how the shell finds commands through `PATH`.

**Day 4 — Troubleshooting the environment**  
They apply networking tools, firewall concepts and logs, then verify that WSL2 itself is ready for the database and dashboard layers.
